...
_REPO_DB_PATH = PROJECT_ROOT / "data" / "ctdt_sis_v3.db"
_LEGACY_DB_PATH = PROJECT_ROOT / "data" / "course_registration.db"
DEFAULT_DB_PATH = Path(
    os.getenv(
        "COURSE_REGISTRATION_DB_PATH",
        str(_LOCAL_FIXED_DB_PATH if _LOCAL_FIXED_DB_PATH.exists() else (_REPO_DB_PATH if _REPO_DB_PATH.exists() else _LEGACY_DB_PATH)),
    )
)
...
def passed_courses_source(conn: sqlite3.Connection) -> str:
    ...
    if table_or_view_exists(conn, "v_ket_qua_hoc_tap_sv"):
        return "v_ket_qua_hoc_tap_sv"
